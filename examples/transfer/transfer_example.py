...
from netsalt.transfer import get_node_transfer_matrix, get_static_boundary_flow, get_edge_transfer_matrix
...
if __name__ == "__main__":
    ...
    for k in ks:
        #r = get_node_transfer_matrix(k, graph, input_flow)[ids]
        r = get_edge_transfer_matrix(k, graph, input_flow)[e_ids]
        res.append(r)

    plt.figure()
    ax = plt.gca()
    f = lambda x: np.abs(x)
    ax.plot(ks, f(np.array(res)[:, 0]), label="0")
    ax.plot(ks, f(np.array(res)[:, 1]), label="1")
    #ax.plot(ks, f(np.array(res)[:, 0])-f(np.array(res)[:, 1]), label="all")
    #ax.plot(ks, f(np.array(res)[:, 2]), label="2")
